   ...
   private function generateJobNumber()
    ...
    private function createJobOrder($plp, $noJob, $request)
    {
        ...
        return Job::create([
            ...
            'ttgl_bc11' => Carbon::parse($plp->tgl_bc11)->format('Y-m-d'),
            'noplp' => $plp->no_plp,
            'ttgl_plp' => Carbon::parse($plp->tgl_plp)->format('Y-m-d'),
            'type' => 'lcl',
            'uid' => Auth::user()->id,
    ...
    private function createContainers($plpDetails, $job, $ctr_type)
    {
        ...
        foreach ($conts as $cont) {
            ...
            Cont::create([
                ...
                'uid' => Auth::user()->id,
                'nobl' => $cont->no_bl_awb,
                'tgl_bl_awb' => $cont->tgl_bl_awb ? Carbon::parse($cont->tgl_bl_awb)->format('Y-m-d') : null,
                'eta'=> $job->eta,
