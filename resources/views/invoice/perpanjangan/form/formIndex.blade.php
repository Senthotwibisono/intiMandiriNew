...
<form action="/invoice/form/perpanjangan/submitStep1" method="post" enctype="multipart/form-data">
    @csrf
    <section>
        <div class="card">
                ...
                <div class="row mt-0">
                    <div class="col-sm-2">
                        <div class="form-group">
                            ...
                            <select name="old_invoice_id" id="old_invoice_id" style="width:100%;" class="js-example-basic-single select2 form-select">
                                <option disabled selected value>Pilih Satu !</option>
                                @foreach($oldInvoices as $inv)
                                    <option value="{{$inv->id}}" {{$form->old_invoice_id == $inv->id ? 'selected' : ''}}>{{$inv->invoice_no}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    <div class="col-sm-2">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" name="nohbl" id="nohbl" value="{{$form->manifest->nohbl ?? ''}}" readonly>
                        </div>
                    </div>
                    <div class="col-sm-2">
                        <div class="form-group">
    ...
    document.getElementById('back-button').addEventListener('click', function(event) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                window.location.href = '/invoice/form/perpanjangan/index';
            }
        });
    });
</script>
<script>
    $(document).ready(function() {
        $('#old_invoice_id').on('change', function() {
            var manifestId = $(this).val();

            if (manifestId) {
                $.ajax({
                    url: '/get-oldInvocie-data/' + manifestId,
                    type: 'GET',
                    dataType: 'json',
