    ...
    <div class="container">
        ...
        <div class="card-body">
            ...
            @foreach($containers as $cont)
                <div class="row">
                    <div class="col-auto">
                        Contianer/Size/Type : <b>{{$cont->cont->nocontainer ?? '-'}}/{{$cont->size}}/{{$cont->ctr_type ?? '-'}}</b>
                    </div>
                    <div class="col-auto">
                        ETA : <b>{{$header->eta ?? '-'}}</b>
                    </div>
                    <div class="col-auto">
                        Tgl Masuk : <b>{{$cont->tglmasuk ?? '-'}}</b>
                    </div>
                    <div class="col-auto">
                        Rencana Keluar : <b>{{$header->etd ?? '-'}}</b>
                    </div>
                </div>
                    ...
                    <table class="tabel-stripped mx-auto">
                        ...
                        <tbody>
                            @foreach($detilTPS as $tps)
                                @if($tps->size == $cont->size && $tps->type == $cont->ctr_type)
                                @php
                                    $total = $tps->total / $tps->jumlah;
                                    $admin = $header->admin / $containers->count();

                                    $tanggal = null; // Default jika tidak ditemukan
                                    $flag = false;

                                    if (str_contains($tps->keterangan, 'masuk pd ')) {
                                        $parts = explode('masuk pd ', $tps->keterangan);
                                        $flag = true;
                                        $tanggal = trim($parts[1]); // Ambil bagian setelah 'masuk pd'
                                    }
                                @endphp
                                    @if($flag == true)
                                       @if($tanggal == $cont->tglmasuk)
                                        <tr>
                            ...
                            @endforeach
                                <tr>
                                    ...
                                    <td colspan="4">{{ number_format($admin, 0)}}</td>
                                </tr>
                        </tbody>
                    </table>
                </div>
                    ...
                    <table class="tabel-stripped mx-auto">
                        ...
                        <tbody>
                            @foreach($detilWMS as $wms)
                                @if($wms->size == $cont->size && $wms->type == $cont->ctr_type)
                                    @php
                                        $totalWMS = $wms->total / $wms->jumlah;
                                    
                                        $tanggalWMS = null; // Default jika tidak ditemukan
                                        $flagWMS = false;
                                    
                                        if (str_contains($wms->keterangan, 'Masuk pd ')) {
                                            $parts = explode('Masuk pd ', $wms->keterangan);
                                            $flagWMS = true;
                                            $tanggalWMS = trim($parts[1]); // Ambil bagian setelah 'Masuk pd'
                                        }
                                    @endphp
                                        @if($flagWMS == true)
                                            @if($tanggalWMS == $cont->tglmasuk)
                                                <tr>
